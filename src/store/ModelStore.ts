...
import {shouldKeepModelResident} from '../services/localApi/residencyGuard';
...
class ModelStore {
   ...
   * 局域网服务期间的模型常驻（最小改动面）。
   *
   * 只改这一个判定点，且不碰 `useAutoRelease` 用户偏好字段、不碰任何调用点签名：
   * 服务关闭或未开启常驻时，表达式与改造前逐字一致 —— 这是可验证的不变量。
   *
   * 为什么必须有这个开关：原来的逻辑在 AppState 一进 background 就释放上下文。
   * 对单机聊天这是合理的省内存策略，但对对外提供服务的场景是致命的 —— App
   * 一切到后台，正在服务的外部客户端会毫无征兆地开始收到 503，而服务本身还在
   * 监听，用户看到的是「服务开着但不能用」。
   */
